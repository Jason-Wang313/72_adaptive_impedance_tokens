...
\usepackage{url}
\usepackage{booktabs}
\title{Adaptive Impedance Tokens: ICLR Main-Conference Gate Archive}
\author{Anonymous Authors}
\begin{document}
\maketitle
\begin{abstract}
This document is not an ICLR main-conference submission. It is the v3 main-conference gate for a generated robotics draft about impedance control and robot foundation models. The original research bet was: Represent impedance choices as adaptive action tokens grounded in contact outcomes. After a stricter hostile review, the honest terminal decision is \textbf{KILL/ARCHIVE}. Submission-hardening version: v3.
\end{abstract}
...
\begin{itemize}
\item A hybrid control strategy for grinding and polishing robot based on adaptive impedance control (2021)
\item Adaptive Admittance-Impedance Switching Control for Safe Mobile Manipulation: A Door Opening Application with Predictive Anti-Slip Logic (2026)
\item Force Tracking in Impedance Control (1997)
\item Smooth adaptive hybrid impedance control for robotic contact force tracking in dynamic environments (2020)
\item Stable Robust Adaptive Impedance Control of a Prosthetic Leg (2015)
\end{itemize}
