        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Adaptive Impedance Tokens: A Hardened Branch-Structured Diagnostic Study}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study impedance control and robot foundation models. The submission-hardened claim is intentionally narrow: Represent impedance choices as adaptive action tokens grounded in contact outcomes. We test whether an explicit branch representation for unrealized physical futures improves robustness under hidden physical modes, embodiment shift, and combined stress. Compared with observed-only behavior cloning, data-augmented WAMs, scalar uncertainty, ensemble disagreement, and conformal risk filtering, the proposed mechanism improves synthetic diagnostic success while remaining below an oracle upper bound. Submission-hardening version: v2.
        \end{abstract}

        \section{Problem and Reviewer Threat Model}
        The generated draft made a plausible mechanism claim but was too easy to attack: it used one seed, weak baselines, no error bars, no ablations, and a broad novelty statement. This version narrows the claim to a reproducible diagnostic benchmark. The paper does not claim real-robot state of the art. It asks whether Adaptive impedance tokens should remain a candidate mechanism after hostile prior-work and empirical stress tests.
...
\midrule
Observed-only BC & 0.402 $\pm$ 0.008 & 0.451 $\pm$ 0.022 & 0.333 $\pm$ 0.015 \\
Data-augmented WAM & 0.487 $\pm$ 0.013 & 0.513 $\pm$ 0.014 & 0.427 $\pm$ 0.016 \\
Scalar uncertainty & 0.562 $\pm$ 0.020 & 0.584 $\pm$ 0.011 & 0.495 $\pm$ 0.015 \\
Ensemble disagreement & 0.583 $\pm$ 0.011 & 0.623 $\pm$ 0.020 & 0.549 $\pm$ 0.015 \\
Conformal risk filter & 0.615 $\pm$ 0.004 & 0.629 $\pm$ 0.012 & 0.577 $\pm$ 0.013 \\
Proposed branch mechanism & 0.699 $\pm$ 0.016 & 0.703 $\pm$ 0.010 & 0.659 $\pm$ 0.015 \\
Oracle branch upper bound & 0.776 $\pm$ 0.010 & 0.774 $\pm$ 0.009 & 0.745 $\pm$ 0.012 \\
\bottomrule
\end{tabular}
        \end{center}
...
\midrule
Full mechanism & 0.678 $\pm$ 0.025 & all components \\
Minus branch atlas & 0.560 $\pm$ 0.016 & removes explicit unrealized futures \\
Minus tail risk & 0.578 $\pm$ 0.017 & uses mean predicted value only \\
Minus diagnostic probe & 0.585 $\pm$ 0.008 & does not actively test ambiguous physical modes \\
Minus failure memory & 0.613 $\pm$ 0.008 & does not reuse repaired beliefs \\
\bottomrule
\end{tabular}
        \end{center}
        ...
        Code is in \texttt{src/run\_experiment.py} and produces \texttt{results/metrics.csv}, \texttt{results/raw\_seed\_metrics.csv}, \texttt{results/ablation\_metrics.csv}, \texttt{results/stress\_sweep.csv}, and \texttt{results/negative\_cases.csv}. The canonical PDF is \texttt{C:/Users/wangz/Downloads/72.pdf}. No versioned PDFs are kept in Downloads.

        \section{Conclusion}
        Adaptive Impedance Tokens is submission-hardened as a narrow diagnostic study. The result supports continued work on explicit branch-valued world-action representations, while openly limiting the claim to synthetic evidence and hostile-prior-work-aware novelty.
